...
let user = {
    firstName: "Manuelita",
    lastName: "Peguajó",
    email: "manuelitamefui@gmail.com",
    age: 150,
}
...
console.log(adalovelace.nombre)
...
const tecnologiasConocidas = ["JavaScript", "CSS", "HTML", "Bulma"];
...
// Dentro de la función, crear la siguiente lógica:
...
let mostrarTecnologias = (objeto) => {
    if (objeto.sabeProgramar) {
        return (`Hola, mi nombre es ${objeto.firstName} y programo en ${objeto.tecnologiasConocidas}`)
    }
}
...
tecnologiasConocidas.push("Próximamente DOM")
...
newUser.name = {first: "Grace", last: "Hopper"}
...
let mostrarGanadoras = () => {
    let resultado = "";
    for (let i = 0; i < ganadoras.length; i++) {
      resultado = resultado.concat(ganadoras[i].nombre + " ganó la temporada " + ganadoras[i].temporada + "\n")
    }
    return resultado
  }
...
let calcularDuracionTotal = (array) => {

    let duracionTotal = 0;

    for (let i = 0; i < array.length; i++) {
       duracionTotal += array[i].duracion        
    }
...
let calcularPromedioPorCancion = () => {
     return calcularDuracionTotal(nevermind)/5
}
...
let estanActivas = (bandas) => {
    ...
    for (let i = 0; i < bandas.length; i++) {
      if (bandas[i].activa) {
          bandas[i].activa = " está activa desde el año " + bandas[i].fundacion;
          resultado.push(bandas[i].nombre + bandas[i].activa)
      }
      else {
          bandas[i].activa = " no está activa"
          resultado.push(bandas[i].nombre + bandas[i].activa)
      }
